...
(function() {
  ...
  app.get('/', function(req, res) {
    ...
    if ((oauthSession.token == null) && (oauthSession.secret == null)) {
      ...
      return client.auth(oauthSession.token, oauthSession.secret).user("hnbulz1l").find(function(err, body, headers) {
        if (err) {
          console.log(err);
        }
        if (body) {
  ...
  app.get('/shop', function(req, res) {
    ...
    return client.auth(oauthSession.token, oauthSession.secret).shop('CustomGiftWear').find(function(err, body, headers) {
      if (err) {
        console.log(err);
      }
      if (body) {
  ...
  //   var oauthSession, updatedProfile;
  //   oauthSession = {
  //     token: req.session.token,
  //     secret: req.session.sec
  //   };
  ...
  //     city: "New York City"
  //   };
  //   return client.auth(oauthSession.token, oauthSession.secret).user("etsyjs").updateUserProfile(updatedProfile, function(err, body, headers) {
  //     if (err) {
  //       console.log(err);
  //     }
  //     if (body) {
  ...
  //   return client.auth(oauthSession.token, oauthSession.secret).address("etsyjs").delete(12787771379, function(err, body, headers) {
  //     if (err) {
  //       console.log(err);
  //     }
  //     if (body) {
